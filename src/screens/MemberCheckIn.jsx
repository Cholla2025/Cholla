...
// Member Check-In — clients arriving at and leaving the facility for the day
// (their own daily door log, separate from group attendance and completely
// separate from Community visitors). A top-level tab for every staff role;
// the log is date-keyed so it resets at midnight, and anyone never checked
// out on a past day displays as auto-departed.
...
export default function MemberCheckIn({ store }) {
  ...
  const load = async (d) => {
    ...
    else { setRows([]); setErr('Could not load the Member Check-In log — check the connection') }
  }
  ...
  return (
    <div className="scroll fade cholla-scroll">
      <div className="section-title">Member Check-In</div>
      <div className="section-sub">Members arriving at and leaving the facility · {st.todayLabel}</div>
      ...
      <div className="card" style={{ marginTop: 14 }}>
        ...
        <div className="muted" style={{ font: '500 12px Inter' }}>
          The Member Check-In list starts fresh every day at midnight — anyone
          not checked out is recorded as departing at 11:59 PM.
        </div>
      </div>

      <input className="input" style={{ marginTop: 12 }} value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search member name" />
      ...
      <div className="card" style={{ marginTop: 14, padding: shown.length ? '6px 16px' : 16 }}>
        {rows === null ? <Empty>Loading the Member Check-In log…</Empty> : shown.length ? shown.map((r) => {
        ...
        }) : <Empty title={isToday ? 'Nobody has checked in yet' : 'No Member Check-In activity'}>
